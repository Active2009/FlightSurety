...
contract FlightSuretyApp {
    ...
    uint256 public constant INSUREE_MAX_INSURANCE = 1 ether;
    ...
    /**
     * @dev Modifier that requires the caller to be a registered and funded airline
     */
    modifier requireFundedAirline(address caller) {
        ...
        bool registered;
        bool funded;
        (name, exists, registered, funded) = data.getAirline(caller);
        require(registered == true, "Registered can add flights only");
        require(funded == true, "Funded can add flights, only");
        _;
    }
    ...
    function registerFlight(
        ...
        string description,
        uint256 timestamp
    ) external requireFundedAirline(msg.sender) {
        data.registerFlight(msg.sender, flight, description, timestamp);
    }
    ...
    {
        data.registerAirline(airlineAddress, airlineName, msg.sender);
    }
    ...
    function vote(address airlineAddress)
        ...
        requireFundedAirline(msg.sender)
    {
        data.vote(msg.sender, airlineAddress);
    }
    ...
    function fund(address airlineAddress) external payable {
        ...
        data.fund(airlineAddress);
    }

    function withdraw() external payable {
        uint256 amountWithdrawn = data.withdrawAll(msg.sender);
    ...
    {
        return data.getAirline(airlineAddress);
    }
    ...
    function processFlightStatus(
        ...
        bool complete = data.updateFlight(
            airline,
            flight,
            timestamp,
            statusCode
    ...
    function registerOracle() external payable {
        ...
        require(msg.value >= REGISTRATION_FEE, "Fee is required");
